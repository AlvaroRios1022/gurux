    ...
    _ValidateResponse: function(response) {
        ...
        if (response.status == 200) {
            ...
            if (response.status == 0){
                $("#modal_mensaje_confirmacion").modal('toggle')
                $("#mensaje_respuesta").text("No pudo conectarse con el servidor.</br>Por favor, revise su conexión a internet o comuníquese con el administrador.");
            }
            
            else{
                $("#modal_mensaje_confirmacion").modal('toggle')
                $("#mensaje_respuesta").text( response.status + ": " + response.statusText);
            }
    ...
    _UploadFile: function(e, url, callback) {
        ...
        $.ajax({
            ...
            complete: function(jqXHR, textStatus) {
                ...
                if (resultado != null) {
                    ...
                    if (resultado.message != null) {
                        $("#modal_mensaje_confirmacion").modal('toggle')
                        $("#mensaje_respuesta").text(resultado.message);
                    }
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                $("#modal_mensaje_confirmacion").modal('toggle')
                        $("#mensaje_respuesta").text("Error por tamaño o formato no permitido");
                $loading.stop().fadeOut(100);
            }
        });
    }
            ...
            $.fn.dataTable.ext.errMode = function (settings, helpPage, message) {
                ...
                if (settings.jqXHR !== null && typeof (settings.jqXHR.responseJSON) !== 'undefined') {
                   ...
                   // Utils._BuilderMessage('danger', messageError);
                    ...
                    $("#modal_mensaje_confirmacion").modal('toggle')
                        $("#mensaje_respuesta").text('Ocurrió un error, por favor comuníquese con el administrador.');
...
jQuery(document).ready(function($) {
  ...
  var alterClass = function() {
    ...
    if (ww < 400) {
     $("#nombre_usuario_para_mobile").removeClass('nombre_titulo_escritorio')
    

    } else if (ww >= 401) {
        $("#nombre_usuario_para_mobile").addClass('nombre_titulo_mobile')
